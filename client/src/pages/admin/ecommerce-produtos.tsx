...
import { Plus, Edit2, Trash2, Eye, EyeOff, Search, Filter, TrendingUp, Package, DollarSign, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, X } from "lucide-react";
...
export default function AdminProdutos() {
  ...
  const [ordenacao, setOrdenacao] = useState<"nome" | "preco" | "operadora">("nome");
  ...
  const criarMutation = useMutation({
    mutationFn: async (data: Partial<EcommerceProduct>) => {
      const res = await apiRequest("POST", "/api/admin/ecommerce/manage/products", data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/ecommerce/manage/products"] });
  ...
  const atualizarMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<EcommerceProduct> }) => {
      const res = await apiRequest("PUT", `/api/admin/ecommerce/manage/products/${id}`, data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/ecommerce/manage/products"] });
  ...
  const deletarMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiRequest("DELETE", `/api/admin/ecommerce/manage/products/${id}`, {});
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/ecommerce/manage/products"] });
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ...
      precoInstalacao: parseInt(formData.get("precoInstalacao") as string || "0") * 100,
      ...
      permiteCalculadoraLinhas: formData.get("permiteCalculadoraLinhas") === "on",
  ...
  const produtosFiltrados = useMemo(() => {
    ...
    if (filtroTipoPessoa !== "todos") {
      resultado = resultado.filter(p => 
        p.tipoPessoa === filtroTipoPessoa || p.tipoPessoa === "ambos"
      );
    }
    ...
    return resultado;
  }, [produtos, busca, filtroOperadora, filtroCategoria, filtroStatus, filtroTipoPessoa, ordenacao]);
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-slate-600">Gerencie os planos e ofertas disponíveis</p>
          </div>
          <Button onClick={() => { 
            setEditando(null); 
            setSvasSelecionados([]); 
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{formatPreco(stats.precoMedio)}</p>
                </div>
                ...
                <div className="space-y-1">
                  {Object.entries(stats.porOperadora).map(([op, count]) => (
                    <div key={op} className="flex justify-between text-sm">
                      <span className="font-medium">{op === "V" ? "VIVO" : op === "C" ? "CLARO" : "TIM"}:</span>
                      <span>{count}</span>
                    </div>
                  ))}
                </div>
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            {/* Operadora */}
            <div>
              <Select value={filtroOperadora} onValueChange={(v) => { setFiltroOperadora(v); resetPagina(); }}>
                <SelectTrigger>
            ...
            {/* Categoria */}
            <div>
              <Select value={filtroCategoria} onValueChange={(v) => { setFiltroCategoria(v); resetPagina(); }}>
                <SelectTrigger>
            ...
            {/* Tipo Pessoa */}
            <div>
              <Select value={filtroTipoPessoa} onValueChange={(v) => { setFiltroTipoPessoa(v); resetPagina(); }}>
                <SelectTrigger>
          ...
          <div className="mt-4 flex items-center justify-between flex-wrap gap-4">
            <p className="text-sm text-slate-600">
              Mostrando <span className="font-semibold">{indexInicio + 1}</span> - <span className="font-semibold">{Math.min(indexFim, produtosFiltrados.length)}</span> de <span className="font-semibold">{produtosFiltrados.length}</span> produtos
            </p>
            
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-sm text-slate-600">Itens por página:</span>
                <Select value={itensPorPagina.toString()} onValueChange={(v) => { setItensPorPagina(Number(v)); resetPagina(); }}>
                  ...
                  <TableRow key={produto.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline">
                        {produto.operadora === "V" ? "VIVO" : produto.operadora === "C" ? "CLARO" : "TIM"}
                      </Badge>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="text-sm">
                        ...
                        {produto.franquia && <div className="text-slate-600">{produto.franquia}</div>}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="font-semibold">{formatPreco(produto.preco)}</div>
                      {produto.fidelidade > 0 && (
                        <div className="text-xs text-slate-500">{produto.fidelidade} meses</div>
                      )}
                    </TableCell>
                    <TableCell>
                      <Badge variant="secondary">
                        {produto.tipoPessoa === "PF" ? "PF" : produto.tipoPessoa === "PJ" ? "PJ" : "Ambos"}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      {produto.ativo ? (
                      ...
                      <div className="flex justify-end gap-1">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setTextosUpsell((produto.textosUpsell && produto.textosUpsell.length > 0) ? produto.textosUpsell : ['']);
                            setDialogOpen(true);
                          }}
                        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select name="categoria" defaultValue={editando?.categoria || ""}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select name="operadora" defaultValue={editando?.operadora || "V"}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input name="velocidade" defaultValue={editando?.velocidade || ""} placeholder="Ex: 500 Mbps" />
              </div>
              
              <div>
                <Label>Franquia</Label>
                <Input name="franquia" defaultValue={editando?.franquia || ""} placeholder="Ex: Ilimitado" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Preço (R$)</Label>
                <Input name="preco" type="number" step="0.01" defaultValue={editando ? editando.preco / 100 : ""} required />
              </div>
              
              <div>
                <Label>Instalação (R$)</Label>
                <Input name="precoInstalacao" type="number" step="0.01" defaultValue={editando?.precoInstalacao ? editando.precoInstalacao / 100 : 0} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input name="fidelidade" type="number" defaultValue={editando?.fidelidade || 0} />
              </div>
              
              <div>
                <Label>Tipo Pessoa</Label>
                <Select name="tipoPessoa" defaultValue={editando?.tipoPessoa || "ambos"}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div>
              ...
              <Textarea name="beneficios" defaultValue={editando?.beneficios?.join("\n") || ""} rows={4} />
            </div>
            
            {/* NOVOS CAMPOS - SISTEMA DE RECOMENDAÇÃO INTELIGENTE */}
            <div className="border-t pt-4 mt-4">
              <h3 className="text-sm font-semibold mb-3">Sistema de Recomendação Inteligente</h3>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>Modalidade</Label>
                  <Select name="modalidade" defaultValue={editando?.modalidade || "ambos"}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Input name="scoreBase" type="number" min="0" max="100" defaultValue={editando?.scoreBase || 50} />
                </div>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-xs text-slate-500 mt-1">Opções: trabalho, streaming, jogos, basico, equipe</p>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>Limite Mínimo de Dispositivos</Label>
                  <Input name="limiteDispositivosMin" type="number" min="0" defaultValue={editando?.limiteDispositivosMin || ""} />
                </div>
                
                <div>
                  <Label>Limite Máximo de Dispositivos</Label>
                  <Input name="limiteDispositivosMax" type="number" min="0" defaultValue={editando?.limiteDispositivosMax || ""} />
                </div>
              </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <Switch name="ativo" defaultChecked={editando?.ativo !== false} />
                <Label>Ativo</Label>
              </div>
              
              <div className="flex items-center space-x-2">
                <Switch name="destaque" defaultChecked={editando?.destaque || false} />
                <Label>Destaque</Label>
              </div>

              <div className="flex items-center space-x-2">
                <Switch name="permiteCalculadoraLinhas" defaultChecked={editando?.permiteCalculadoraLinhas || false} />
            ...
            <div className="space-y-3">
                  ...
                  <div key={index} className="flex gap-2">
                    ...
                    {textosUpsell.length > 1 && (
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => setTextosUpsell(textosUpsell.filter((_, i) => i !== index))}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    )}
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                ...
                <p>• Use variáveis: <code className="bg-slate-100 px-1 rounded">[nome_servico]</code> e <code className="bg-slate-100 px-1 rounded">[preco]</code></p>
            ...
            <div className="space-y-3">
              ...
              <div className="border rounded-lg p-4 max-h-[200px] overflow-y-auto bg-slate-50 space-y-2">
                {produtosSVA.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-4">
                    Nenhum produto SVA cadastrado ainda. Crie produtos com categoria "sva" primeiro.
                  </p>
                ) : (
                  produtosSVA.map((sva) => (
                    <div
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            ...
                            setSvasSelecionados(svasSelecionados.filter(id => id !== sva.id));
                          }
                        }}
                      />
                      <label
                      ...
                      >
                        <div className="font-medium text-slate-900">{sva.nome}</div>
                        <div className="text-xs text-slate-600">
                          {(sva.preco / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}/mês
                          {sva.descricao && ` • ${sva.descricao.substring(0, 50)}...`}
                        </div>
                      </label>
                    </div>
                  ))
              ...
              {svasSelecionados.length > 0 && (
                <p className="text-xs text-muted-foreground">
                  {svasSelecionados.length} SVA{svasSelecionados.length > 1 ? 's' : ''} selecionado{svasSelecionados.length > 1 ? 's' : ''}
                </p>
              )}
            </div>
