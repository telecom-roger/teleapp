...
export default function AdminEcommercePedidos() {
  ...
  const updateEtapaMutation = useMutation({
    mutationFn: async ({ orderId, etapa, observacoes }: { orderId: string; etapa: string; observacoes?: string }) => {
    ...
    onSuccess: () => {
      ...
      if (selectedOrder) {
        queryClient.invalidateQueries({ queryKey: [`/api/admin/ecommerce/orders/${selectedOrder.id}`] });
      }
      toast({
  ...
  const groupedOrders = useMemo(() => {
    return etapas.reduce((acc, etapa) => {
      acc[etapa.value] = pedidosFiltrados.filter(o => o.etapa === etapa.value);
      return acc;
  ...
  const handleDrop = (toEtapa: string) => {
    ...
    const fromEtapaLabel = etapas.find(e => e.value === draggedOrder.fromEtapa)?.label;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats?.porEtapa.find(e => e.etapa === "aguardando_documentos")?.count || 0}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
              <Select value={filtroEtapa} onValueChange={(v) => { setFiltroEtapa(v); resetPagina(); }}>
                <SelectTrigger>
            ...
            <div>
              <Select value={filtroPeriodo} onValueChange={(v) => { setFiltroPeriodo(v); resetPagina(); }}>
                <SelectTrigger>
          ...
          <div className="mt-4">
            <p className="text-sm text-slate-600">
              Mostrando <span className="font-semibold">{pedidosFiltrados.length}</span> de <span className="font-semibold">{orders?.length || 0}</span> pedidos
            </p>
          </div>
        </CardContent>
      </Card>
            ...
            >
              ...
              >
                <div className="space-y-2 max-h-[600px] overflow-y-auto">
                  {ordersInEtapa.length === 0 && dragOverEtapa === etapa.value && (
                          ...
                          <TableCell>
                            <div>
                              <div className="font-medium">
                                {order.nomeCompleto || order.razaoSocial || "N/A"}
                              </div>
                              <div className="text-xs text-slate-500">
                                {order.itemsCount || 0} {order.itemsCount === 1 ? "item" : "itens"}
                              </div>
                            </div>
                          </TableCell>
                          <TableCell>
              ...
              {totalPaginas > 1 && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t">
                  <div className="flex items-center gap-4">
                    <p className="text-sm text-slate-600">
                      Mostrando <span className="font-semibold">{indexInicio + 1}</span> - <span className="font-semibold">{Math.min(indexFim, pedidosFiltrados.length)}</span> de <span className="font-semibold">{pedidosFiltrados.length}</span>
                    </p>
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Pedido recebido em {selectedOrder && new Date(selectedOrder.createdAt).toLocaleString("pt-BR")}
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{orderDetails.nomeCompleto || orderDetails.razaoSocial}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Tipo</p>
                    <p className="font-medium">{orderDetails.tipoPessoa === "PF" ? "Pessoa Física" : "Pessoa Jurídica"}</p>
                  </div>
                  <div>
                  ...
                  {orderDetails.items?.map((item) => (
                    <div key={item.id} className="flex justify-between p-3 border rounded-lg">
                      ...
                      <div className="text-right">
                        <p className="font-medium">R$ {(item.subtotal / 100).toFixed(2)}</p>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    className="w-full"
                  >
                    {updateEtapaMutation.isPending ? "Atualizando..." : "Atualizar Etapa"}
                  </Button>
                </div>
              </div>
