...
import { recomendarProdutos, type FiltrosRecomendacao } from "@/lib/recomendacao";
...
export default function CategoriaPage() {
  ...
  const { data: categoria } = useQuery<EcommerceCategory>({
    ...
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/ecommerce/public/categories/${slug}`, {});
      return res.json();
    },
    enabled: !!slug,
  });
  ...
  const { data: produtos = [] } = useQuery<EcommerceProduct[]>({
    ...
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/ecommerce/public/products?categoria=${slug}`, {});
      return res.json();
    },
    enabled: !!slug,
  });
  ...
  const categoriaInfo = {
    fibra: {
      ...
      description: "Navegue, trabalhe e assista com a velocidade que você merece. Conexões estáveis de até 1 Gbps.",
      highlights: ["Ultra velocidade", "Estabilidade", "Fibra 100%"],
    },
    movel: {
      title: "Planos Móveis com Internet Ilimitada",
      description: "Fique conectado onde estiver com nossos planos de celular com apps ilimitados e cobertura nacional.",
      highlights: ["Apps ilimitados", "Cobertura 4G/5G", "Portabilidade grátis"],
    },
    aparelhos: {
      title: "Smartphones e Aparelhos",
      description: "Os melhores dispositivos com parcelamento sem juros e entrega rápida.",
      highlights: ["Parcelamento 12x", "Entrega rápida", "Garantia estendida"],
    },
    combo: {
      title: "Combos Fibra + TV",
      description: "Economize combinando internet ultra rápida com TV por assinatura com canais HD.",
      highlights: ["Economia", "Canais HD", "Instalação única"],
    },
    office365: {
      title: "Microsoft Office 365 para Empresas",
      description: "Produtividade na nuvem com Word, Excel, PowerPoint, Teams e 1TB de armazenamento.",
      highlights: ["Apps completos", "1TB OneDrive", "Suporte técnico"],
    },
    "internet-dedicada": {
      title: "Internet Dedicada Empresarial",
      description: "Banda dedicada com SLA 99.5%, IP fixo e suporte 24/7 para sua empresa.",
      highlights: ["SLA 99.5%", "IP fixo", "Suporte 24/7"],
    },
    pabx: {
      title: "PABX Virtual na Nuvem",
      description: "Sistema de telefonia empresarial completo sem necessidade de equipamentos físicos.",
      highlights: ["Sem hardware", "Ramais ilimitados", "Integração CRM"],
    },
    locacao: {
      title: "Locação de Equipamentos",
      description: "Alugue roteadores, switches e equipamentos de rede com manutenção inclusa.",
      highlights: ["Manutenção inclusa", "Sem investimento", "Upgrade facilitado"],
    },
  };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-gradient-to-br from-primary/10 via-secondary/10 to-accent/10 py-16">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <p className="text-lg text-slate-600 mb-6 max-w-2xl">{info.description}</p>
          
          <div className="flex gap-3">
            {info.highlights.map((highlight, i) => (
              <Badge key={i} className="bg-white/90 text-primary border-primary/20">
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() =>
                  setFiltros({
                    ...filtros,
                    modalidade: filtros.modalidade === "novo" ? undefined : ("novo" as const),
                  })
                }
              >
                Novo Número
              ...
              <Button
                variant={filtros.modalidade === "portabilidade" ? "default" : "outline"}
                size="sm"
                onClick={() =>
                  setFiltros({
                    ...filtros,
                    modalidade: filtros.modalidade === "portabilidade" ? undefined : ("portabilidade" as const),
                  })
                }
              >
                Portabilidade
          ...
          <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {filtros.operadora && (
                  <Badge variant="secondary" className="font-semibold">
                    {filtros.operadora === "V" ? "VIVO" : filtros.operadora === "C" ? "CLARO" : "TIM"}
                  </Badge>
                )}
              </div>
            </div>
            ...
            {produtosRecomendados.length === 0 && (
              <Card className="p-12 text-center">
                ...
                <Button onClick={() => setFiltros({ tipoPessoa: "PF", modalidade: undefined, operadora: undefined })}>
                  Limpar Filtros
                </Button>
              </Card>
            )}
        ...
        <div className="bg-white border-t py-16">
          <div className="max-w-7xl mx-auto px-4">
            <div className="text-center mb-8">
              ...
              <p className="text-slate-600">
                Personalize seu plano com serviços adicionais. Use o botão "Nova Linha" no card do plano para adicionar linhas.
              </p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              {adicionais.slice(0, 8).map((adicional: any) => (
                <Card key={adicional.id} className="p-4 hover:shadow-lg transition-shadow">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h3 className="font-semibold text-sm mb-1">{adicional.nome}</h3>
                  ...
                  <p className="text-xs text-slate-600 mb-3 line-clamp-2">{adicional.descricao}</p>
