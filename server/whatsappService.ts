              ...
              try {
                ...
                if (mensagemAutomatica) {
                  ...
                  if (ultimaMsgAutomatica.length === 0) {
                    ...
                    console.log(`🤖 [AUTOMATION] Salvando mensagem de automação: createdAt=${createdAtTime.toISOString()}`);
                    const [savedMessage] = await db.insert(messages).values({
                      ...
                      createdAt: createdAtTime, // Garantir UTC
                    }).returning();
                    console.log(`💬 [CHAT] Mensagem de automação salva com tracking: ${savedMessage.id}, createdAt no DB: ${savedMessage.createdAt.toISOString()}`);
