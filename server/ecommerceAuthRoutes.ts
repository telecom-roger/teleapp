...
router.post("/login", async (req: Request, res: Response, next) => {
  ...
  if (!identifier || !password) {
    return res
      .status(400)
      .json({ error: "Identificador e senha são obrigatórios" });
  }
  ...
  try {
    ...
    if (isDocument) {
      ...
      console.log(
        "[E-commerce Login] Cliente encontrado:",
        client ? client.id : "Não encontrado"
      );

      if (!client) {
        return res
          .status(401)
          .json({ error: "CPF/CNPJ não encontrado no sistema" });
      }
      ...
      console.log(
        "[E-commerce Login] User vinculado encontrado:",
        user ? user.id : "Não encontrado"
      );
    } else {
      // Login por email
      [user] = await db
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!user.clientId) {
        return res
          .status(400)
          .json({ error: "Usuário não vinculado a um cliente" });
      }
