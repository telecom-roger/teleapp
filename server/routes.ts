...
export async function registerRoutes(
  ...
  // ==================== CLIENT ROUTES ====================
  app.get("/api/clients", isAuthenticated, blockCustomers, async (req, res) => {
    try {
      const user = req.user as any;
      const {
        search,
      ...
      try {
        ...
        const opportunityData = {
          ...
          etapa: validatedData.etapa
            ? validatedData.etapa.toUpperCase()
            : "LEAD", // Normalizar etapa para MAIÚSCULA
        };
        ...
        if (opportunity.clientId) {
          ...
          await storage.updateClient(opportunity.clientId, {
            status: newStatus,
          });
        }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        if (
          validatedData.etapa &&
          oldOpportunity.etapa !== validatedData.etapa
        ) {
          await storage.recordEtapaChange(
            req.params.id,
            oldOpportunity.clientId || "",
  ...
  app.get(
    "/api/timeline/:clientId",
    isAuthenticated,
    blockCustomers,
    async (req, res) => {
      try {
        const timeline = await storage.getTimelineByClientId(
          req.params.clientId
        );
        res.json(timeline);
      } catch (error: any) {
        ...
        for (let i = 0; i < preview.length; i++) {
          try {
            ...
            const clientData = {
              ...
              telefone2:
                row.telefone_2 || row.telefone2 || row.Telefone2 || null,
      ...
      try {
        ...
        try {
          for (const session of sessions) {
            ...
            let liveStatus = whatsappService.getSessionStatus(
              session.sessionId
            );
    ...
    async (req, res) => {
      try {
        ...
        if (!nome) {
          return res
            .status(400)
            .json({ error: "Nome da sessão é obrigatório" });
        }
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `📤 [FORWARD] Encaminhando mensagem para ${normalizedPhone}`
        );
    ...
    async (req, res) => {
      try {
        ...
        if (!conv) {
          ...
          return res
            .status(404)
            .json({ error: "Conversation not found", logs });
        }
        logs.push({
          step: 1,
          status: "OK",
  ...
  // ==================== TAGS ROUTES ====================
  app.get("/api/tags", isAuthenticated, blockCustomers, async (req, res) => {
    try {
      const user = req.user as any;
  ...
  app.post("/api/tags", isAuthenticated, blockCustomers, async (req, res) => {
    try {
      const user = req.user as any;
