...
export function registerEcommerceRoutes(app: Express): void {
  ...
  app.get("/api/ecommerce/products", async (req, res) => {
    try {
      ...
      if (categoria) conditions.push(eq(ecommerceProducts.categoria, categoria as string));
      if (operadora) conditions.push(eq(ecommerceProducts.operadora, operadora as string));
      if (tipoPessoa) {
        conditions.push(
      ...
      const produtos = await query.orderBy(
        ecommerceProducts.destaque ? desc(ecommerceProducts.destaque) : ecommerceProducts.ordem
      );
  ...
  app.delete("/api/ecommerce/products/:id", isAuthenticated, async (req, res) => {
    try {
      await db
  ...
  app.put("/api/ecommerce/stages/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      res.status(400).json({ error: error.message || "Erro ao atualizar stage" });
    }
  });
  ...
  app.get("/api/ecommerce/orders", isAuthenticated, async (req, res) => {
    try {
      ...
      if (tipoPessoa) conditions.push(eq(ecommerceOrders.tipoPessoa, tipoPessoa as string));
  ...
  app.post("/api/ecommerce/orders", async (req, res) => {
    try {
      ...
      let senhaTemporaria = "";
      const documento = orderData.tipoPessoa === "PF" ? orderData.cpf : orderData.cnpj;
      ...
      if (existingClients.length > 0) {
        ...
        const [novoCliente] = await db
          .insert(clients)
          .values({
            nome: orderData.tipoPessoa === "PF" ? orderData.nomeCompleto : orderData.razaoSocial,
        ...
        if (existingUser) {
          ...
          await db.insert(users).values({
            email: loginEmail,
            passwordHash,
            firstName: orderData.tipoPessoa === "PF" ? orderData.nomeCompleto?.split(" ")[0] : orderData.razaoSocial,
            lastName: orderData.tipoPessoa === "PF" ? orderData.nomeCompleto?.split(" ").slice(1).join(" ") : "",
            role: "customer",
            clientId: clientId,
            active: true,
          });
          ...
          if (isEmailConfigured()) {
            ...
            }).catch((err: unknown) => console.error("Erro ao enviar email de boas-vindas:", err));
          }
      ...
      if (orderData.items && orderData.items.length > 0) {
        ...
        await notifyNewOrder(
          order.id,
          orderData.tipoPessoa === "PF" ? orderData.nomeCompleto : orderData.razaoSocial,
          order.total
        ).catch((err: unknown) => console.error("Erro ao notificar admins:", err));
        ...
        if (isEmailConfigured()) {
          ...
          }).catch((err: unknown) => console.error("Erro ao enviar email de pedido:", err));
        }
      }
      
      res.json({ success: true, orderId: order.id, clientId });
    } catch (error: any) {
  ...
  app.put("/api/ecommerce/orders/:id/status", isAuthenticated, async (req, res) => {
    try {
      ...
      if (isEmailConfigured() && order.email) {
        enviarEmailStatusPedido({
          ...
          novoStatus: status,
        }).catch((err: unknown) => console.error("Erro ao enviar email de status:", err));
      }
  ...
  app.post("/api/ecommerce/setup", isAuthenticated, async (req, res) => {
    try {
      ...
      try {
        await db.execute(sql`ALTER TABLE clients ADD COLUMN IF NOT EXISTS origin VARCHAR(20) DEFAULT 'system'`);
        await db.execute(sql`UPDATE clients SET origin = 'system' WHERE origin IS NULL`);
      ...
      res.json({ 
        ...
        produtos: produtos.map(p => ({ id: p.id, nome: p.nome, preco: p.preco / 100 }))
      });
    } catch (error: any) {
  ...
  app.post("/api/ecommerce/orders/:id/documents", async (req, res) => {
    try {
      ...
      const filePath = `/uploads/orders/${req.params.id}/${Date.now()}_${fileName}`;
  ...
  app.post("/api/ecommerce/customer/orders/:orderId/mark-viewed", isAuthenticated, async (req, res) => {
    try {
  ...
  app.post("/api/ecommerce/customer/orders/mark-all-viewed", isAuthenticated, async (req, res) => {
    try {
      const user = req.user as any;
