...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const user = req.user as any;
      const { endereco, numero, complemento, bairro, cidade, uf, cep } =
        req.body;
      ...
      const requestNote = `\n[SOLICITAÇÃO DE MUDANÇA DE ENDEREÇO - ${new Date().toLocaleString(
        "pt-BR"
      ...
      res.json({
        success: true,
        message:
          "Solicitação de mudança de endereço enviada. Aguarde aprovação da equipe.",
      });
    } catch (error: any) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!orderId || !tipo) {
        return res
          .status(400)
          .json({ error: "orderId e tipo são obrigatórios" });
      }
