 ...
 */
export function blockCustomers(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  if (!req.isAuthenticated || !req.isAuthenticated()) {
    return res.status(401).json({ error: "Não autenticado" });
  }
  ...
  if (user && user.role === 'customer') {
    return res.status(403).json({ 
      error: "Acesso negado",
      message: "Clientes não podem acessar o sistema administrativo. Acesse o painel do cliente em /ecommerce/painel" 
    });
  }
 ...
 */
export function requireOwnData(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  if (!req.isAuthenticated || !req.isAuthenticated()) {
    return res.status(401).json({ error: "Não autenticado" });
  }
